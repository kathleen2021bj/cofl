...
if __name__ == "__main__":
    ...
    for le in range(length):
        print('parameters: %d' % (le+1))
        print(list(net.parameters())[le].size())
    ...
    # print('last_acc_list', last_acc_list)
    # print(max(last_acc_list[0]))
    # a = max(last_acc_list[0])
    # print(type(a))
    ...
    for i in range(args['num_comm']):
        ...
        for client in tqdm(clients_in_comm):
            ...
            # print('max(last_acc_list[jishuqi]):', max(last_acc_list[jishuqi]))
            # print('type max(last_acc_list[jishuqi])', type(max(last_acc_list[jishuqi])))
            # print(last_acc_list[duitou])
            # print('last_acc_list', last_acc_list)
            if last_acc < max(last_acc_list[duitou]):
                ...
                if jishuqi == 0:
                    ...
                    node21.append('1')
                # last_acc_list[jishuqi].remove(max(last_acc_list[jishuqi]))
                # last_acc_list[jishuqi].append(last_acc)

                # last_acc_list[jishuqi] = last_acc
                baocun_b = []
                for ij in range(5):
        ...
        node8_ssry.append(last_acc_list[2])
        node15_ssry.append(last_acc_list[15])
        node21_ssry.append(last_acc_list[21])


        for var in global_parameters:
            global_parameters[var] = (sum_parameters[var] / num_in_comm)

        precision = torchmetrics.Precision(average='none', num_classes=12).to(dev)
        test_recall = torchmetrics.Recall(average='none', num_classes=12).to(dev)
        test_auc = torchmetrics.AUROC(average="macro", num_classes=12).to(dev)
    ...
    print('node8ssry', node8_ssry)
    print('node15ssry', node15_ssry)
    print('node21ssry', node21_ssry)
