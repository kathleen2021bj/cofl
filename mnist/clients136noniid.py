    ...
    def dataSetBalanceAllocation(self):
        ...
        for i in range(self.num_of_clients):
            ...
            if i < 2:                # 2 3000
                ...
                shard_size = 100
                j = i - 16
                data_shards = train_data1[j * shard_size: j * shard_size + shard_size]
                label_shards = train_label1[j * shard_size: j * shard_size + shard_size]
                local_data, local_label = np.vstack(data_shards), np.vstack(label_shards)
                local_label = np.argmax(local_label, axis=1)
                # someone = client(TensorDataset(torch.tensor(local_data), torch.tensor(local_label)), self.dev,)
                someone = client(TensorDataset(torch.tensor(local_data), torch.tensor(local_label)), self.dev)
                ...
                shard_size = 300
                j = i - 36
                data_shards = train_data1[1500 + j * shard_size: 1500 + j * shard_size + shard_size]
                label_shards = train_label1[1500 + j * shard_size: 1500 + j * shard_size + shard_size]
                local_data, local_label = np.vstack(data_shards), np.vstack(label_shards)
                local_label = np.argmax(local_label, axis=1)
                # someone = client(TensorDataset(torch.tensor(local_data), torch.tensor(local_label)), self.dev,)
                someone = client(TensorDataset(torch.tensor(local_data), torch.tensor(local_label)), self.dev)
                ...
                shard_size = 500
                j = i - 96
                data_shards = train_data1[19500 + j * shard_size: 19500 + j * shard_size + shard_size]
                label_shards = train_label1[19500 + j * shard_size: 19500 + j * shard_size + shard_size]
                local_data, local_label = np.vstack(data_shards), np.vstack(label_shards)
                local_label = np.argmax(local_label, axis=1)
                # someone = client(TensorDataset(torch.tensor(local_data), torch.tensor(local_label)), self.dev,)
                someone = client(TensorDataset(torch.tensor(local_data), torch.tensor(local_label)), self.dev)
